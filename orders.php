...
<head>
	...
	<title>Orders | Cookbook Ordering System</title>
	<!-- Bootstrap -->
	...
	<div class="row">
		<div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
			<div class="page-header">
				<h2 style="color: whitesmoke; text-align: center;">Create New Order</h2>
			</div>
			<form action="orders.php" method="post" class="form-horizontal">
				<div class="form-group">
					<label for="orderid" class="col-sm-3 control-label">ID</label>
					<div class="col-sm-9">
						<input name="oid" type="text" class="form-control" id="orderid" placeholder="Auto-generated Order ID"readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_num']; ?>">
					</div>
				</div>
				<div class="form-group">
					<label for="orderdate" class="col-sm-3 control-label">Date</label>
					<div class="col-sm-9">
						<input name="orderdate" type="text" class="form-control" id="orderdate" placeholder="Auto generated Order Date" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>">
					</div>
				</div>
				<div class="form-group">
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-10 col-md-offset-1">
      ...
      $start_from = ($page-1) * $per_page;
			try {
				$conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
				$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$sql = "SELECT * FROM tbl_orders_a181765, tbl_staffs_a181765_pt2, tbl_customers_a181765_pt2 WHERE ";
				$sql = $sql."tbl_orders_a181765.fld_staff_id = tbl_staffs_a181765_pt2.fld_staff_id and ";
				$sql = $sql."tbl_orders_a181765.fld_cust_id = tbl_customers_a181765_pt2.fld_cust_id LIMIT $start_from, $per_page";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-1">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a181765");
            $stmt->execute();
            $result = $stmt->fetchAll();
